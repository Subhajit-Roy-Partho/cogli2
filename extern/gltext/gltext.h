...
#ifdef __GNUC__
#define UNUSED_FUCTION [[gnu::unused]]
#else
#define UNUSED_FUCTION
#endif
...
UNUSED_FUCTION GLT_API const char* gltGetText(GLTtext *text)
{
	if (text && text->_text)
	return text->_text;
...
UNUSED_FUCTION GLT_API void gltDrawText(GLTtext *text, const GLfloat mvp[16])
{
	if (!text)
	return;
...
UNUSED_FUCTION GLT_API void gltDrawText2DAligned(GLTtext *text, GLfloat x, GLfloat y, GLfloat scale, int horizontalAlignment, int verticalAlignment)
{
	if (!text)
	return;
...
UNUSED_FUCTION GLT_API void gltDrawText3D(GLTtext *text, GLfloat x, GLfloat y, GLfloat z, GLfloat scale, GLfloat view[16], GLfloat projection[16])
{
	if (!text)
	return;
...
UNUSED_FUCTION GLT_API void gltGetColor(GLfloat *r, GLfloat *g, GLfloat *b, GLfloat *a)
{
	GLfloat color[4];
...
UNUSED_FUCTION GLT_API GLint gltCountSupportedCharacters(const char *str)
{
	if (!str)
	return 0;
...
UNUSED_FUCTION GLT_API void _gltGetViewportSize(GLint *width, GLint *height)
{
	GLint dimensions[4];
