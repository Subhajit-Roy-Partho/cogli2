...
DNAParser::DNAParser(string topology, const std::queue<string> &input_files, std::string end_handling) :
				Parser(input_files) {
	_read_topology(topology);

	if(end_handling == "normal") {
		_dir_handling = DNAStrand::direction_handling::NORMAL;
	}
	else if(end_handling == "size") {
		_dir_handling = DNAStrand::direction_handling::SIZE;
	}
	else if (end_handling == "opacity") {
		_dir_handling = DNAStrand::direction_handling::OPACITY;
	}
	else {
		Logging::log_critical("Invalid keyword %s passed to the --end-handling/-e option. The only accepted keywords are 'normal', 'size' and 'opacity'\n", end_handling.c_str());
		exit(1);
	}
}
	...
	try {
		...
		while(!done) {
			...
			if(buff[0] == 't' && spl.size() == 3) {
				...
				Nucleotide *new_nucleotide = new Nucleotide(pos, v1, v3, color, _base_id[i_nuc]);
				new_nucleotide->set_type(_types[i_nuc]);
				nucleotides.push_back(new_nucleotide);
			}
			curr_line++;
			i_nuc++;
		...
		for(it = nucleotides.begin(); it != nucleotides.end(); it++) {
			...
			if(strands.find(strandid) == strands.end()) {
				strands[strandid] = new DNAStrand();
				strands[strandid]->set_direction_handling(_dir_handling);
			}
		...
		while(sit != strands.end()) {
			sit->second->finalise();
			if(_strand_circularity[sit->first]) {
				sit->second->set_as_circular();
			}
...
void DNAParser::_read_topology(std::string filename) {
	...
	for(int i = 0; i < _N_nuc; i++) {
		...
		if(buff.size() == 0 && !_current_input.good()) {
		...
		if(spl.size() != 4) {
			Logging::log_critical("Malformed topology line '%s'", buff.c_str());
			exit(1);
		}
		...
		if(p == -1 || q == -1) {
			...
			if(p == -1) {
				_types[i] = Nucleotide::type::END_3;
			}
			else {
				_types[i] = Nucleotide::type::END_5;
			}
		}
		else {
			_types[i] = Nucleotide::type::NO_END;
		}
	}
