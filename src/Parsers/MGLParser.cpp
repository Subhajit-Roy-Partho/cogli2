...
Shape *MGLParser::_build_shape(vector<string> &spl) {
	if(spl.size() < 6) {
		throw string(_current_input_name + ": syntax error in line " + StringHelper::to_string<int>(_curr_line) + "\n");
	}
	...
	if(key == "S") {
		...
		vec3 axis1, axis2, abc;
		if(!StringHelper::get_vec3(spl, 7, abc)) {
			throw string("Cannot cast the semiaxes of the Ellipsoid)\n");
		}
		if(!StringHelper::get_vec3(spl, 10, axis1)) {
			throw string("Cannot cast the components of the Ellipsoid axes1\n");
		}
		if(!StringHelper::get_vec3(spl, 13, axis2)) {
			throw string("Cannot cast the components of the Ellipsoid axes2\n");
		}
		res = new Ellipsoid(pos, r, axis1, axis2, abc[0], abc[1], abc[2], color);
	}
	...
	try {
		...
		while(!done) {
			...
			if(buff[0] == '.' && spl.size() == 2) {
				...
				// if the line is not empty and it does not start with a #
				if(buff.size() > 0 && buff[0] != '#') {
					Shape *new_shape;

					spl = StringHelper::split(buff, 'G');
					if(spl.size() > 1) {
						Group *new_group = new Group();
						for(auto token : spl) {
							auto shape_spl = StringHelper::split(token);
							new_group->add_shape(_build_shape(shape_spl));
						}
						new_shape = new_group;
					}
					else {
						spl = StringHelper::split(buff);
						new_shape = _build_shape(spl);

						vec3 pos = new_shape->position();
						for(int d = 0; d < 3; d++) {
							if(pos[d] < min_pos[d]) {
								min_pos[d] = pos[d];
							}
					...
					new_shape->set_id(curr_id);
					curr_id++;
					my_scene->add_shape(new_shape);
				}
			}
			_curr_line++;
