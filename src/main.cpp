...
{ONLY_BLENDER          , 0, "",  "only-blender"  , option::Arg::None,    "  --only-blender\n          Generate a blender script and exit"},
{COM_CENTRE            , 0, "v", "com-centre"    , option::Arg::None,     "  --com-centre, -v\n          Centre the first configuration's centre of mass"},
{ALWAYS_CENTRE         , 0, "" , "always-centre" , option::Arg::Optional, "  --always-centre [<index>]\n          Configurations are centred each time they are visualised with respect to their centre of mass (if no arguments are given) or to a specific particle/strand (if the <index> argument is given)"},
...
{PATCHY_SWAP           , 0, "" , "patchy-swap"   , option::Arg::Required, "  --patchy-swap <delta,cosmax>\n          Assumes a patchy swap configuration, as generated by oxDNA. Requires a topology file. Expects a comma-separated list of parameters (radial and angular widths)"},
...
{NUCLEOTIDE_COLOR_DRUMS, 0, "" , "drums-full"	 , option::Arg::None,     "  --drums-full\n          Color-code DNA nucleotides (both backbone and base) depending on chemical identity, according to the DRuMS color scheme: adenine is blue, guanine is green, cytosine is red, thymine is yellow, uracil is orange."},
{ACTION_FILE		   , 0, "a", "actions-from"	 , option::Arg::Required, "  --actions-from, -a <action_file>\n          A file containing the list of actions that will be performed on the single configurations. Each action will effectively become a new configuration. '+' and '-' can be used to navigate back and forth through actions"},
...
{NO_BOX				   , 0, "" , "no-box"		 , option::Arg::None,     "  --no-box\n          Hide the box"},
{END_HANDLING		   , 0, "e", "end-handling"	 , option::Arg::Required, "  --end-handling, -e <keyword>\n          Set how the appearance of the strand direction (5' -> 3') of DNA and RNA strands is handled. The supported keywords are 'normal' (default, no change in appearance), 'size' (the 5' end backbone is bigger, the 3' end backbone is smaller) and 'opacity' (the opacity of the strand decreases 5' -> 3')"},
...
int main(int argc, char *argv[]) {
	...
	else if(options[ONLY_BLENDER]) {
		c1.print_all_blender();
	}
	else {
		c1.start_ogl();
	}
