...
void OGLManager::init(Camera &camera, LightingState &lighting, std::vector<std::shared_ptr<Scene>> &scenes) {
	...
	if(_window == nullptr) {
		glfwTerminate();
		throw string("Failed to open the GLFW window\n");
	}
...
void OGLManager::_render(bool force_scene_update) {
	...
	if(_show_axes) {
		_axes_shader->use();
		_axes_shader->set_vec3("ambient_light", glm::vec3(0.7, 0.7, 0.7));
...
void OGLManager::key_callback(GLFWwindow *window, int key, int scancode, int action, int mods) {
	...
	// switch between the different ways strand directions are handled
	else if(key == GLFW_KEY_E && press) {
		for(auto shape: manager->current_scene()->get_shapes()) {
			// this operation is supported only for DNA strands
			DNAStrand *DNA_shape = dynamic_cast<DNAStrand *>(shape);
			if(DNA_shape != nullptr) {
				DNA_shape->next_direction_handling();
			}
		}
		manager->current_scene()->set_dirty(true);
	}
