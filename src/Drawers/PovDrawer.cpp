...
PovDrawer::PovDrawer() :
		_out(nullptr), _dna_as_ribbon(false), _patchy_as_kf(false), _patch_header_printed(false), _draw_outlines(false), _cast_shadows(false) {
...
void PovDrawer::visit(Ellipsoid &s) {
	printf ("povray for ellipsoids not implemented yet\n");
	return;
}
...
string PovDrawer::_patches_as_textures(PatchyParticle &p, const string &texture_name) {
	...
	string texture_template("		[ %g pigment { color rgbt<%g,%g,%g,%g> } normal { ripples 0.25 frequency 20 turbulence 0.2 lambda 5 } finish { ambient .2 diffuse .6 phong .75 phong_size 25 } ]\n");
	...
	ss << "#declare " << texture_name << " = texture {\n"
			...
			"	texture_map {\n"
			"		[ 0 pigment { color rgbt<" << cp_str << "> } normal { ripples 0.25 frequency 20 turbulence 0.2 lambda 5 } finish { ambient .2 diffuse .6 phong .5 phong_size 15 } ]\n";
	ss << texture_map << "}\n"
			"}\n";
