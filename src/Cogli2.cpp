...
#include "Parsers/PatchySwapParser.h"
...
Cogli1::Cogli1(option::Parser &parser, option::Option *options) :
	...
	if(input_files.size() == 0) {
		...
		if(options[MANFREDO]) {
		...
		else if(options[PATCHY_SWAP]) {
			_parser = std::make_shared<PatchySwapParser>(topology, input_files, options[PATCHY_SWAP].arg);
		}
			...
			std::string end_handling("normal");
			if(options[END_HANDLING]) {
				end_handling = options[END_HANDLING].arg;
			}
			_parser = std::make_shared<DNAParser>(topology, input_files, end_handling);
		}
	}
	else {
		if(options[RBC]) {
	...
	if(options[BASE_COLOR_AS_BACKBONE] && (options[BASE_COLOR_DRUMS] || options[NUCLEOTIDE_COLOR_DRUMS])) {
		Logging::log_critical("--drums (or --drums-full) and --bcab options are incompatible\n");
		exit(1);
	}
	...
	if(options[NUCLEOTIDE_COLOR_DRUMS]) {
		Nucleotide::base_color_mode = Nucleotide::FULL_DRUMS_COLOUR;
	}
	if(options[RNA]) {
...
void Cogli1::_parse_visibility_file(const char *v_file) {
	...
	VisibilityState default_status_to_apply = HIDE;
	...
	while(v.good()) {
		VisibilityState line_default = default_status_to_apply;
...
void Cogli1::print_all_blender() {
	for(auto scene : _scenes) {
		...
		do {
			_blender_manager.print_scene(*scene, _camera, _lighting);
