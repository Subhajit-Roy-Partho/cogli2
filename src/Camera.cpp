...
void Camera::update(InputState &input, float dt) {
	...
	glm::vec3 new_position = _position + delta_pos;
	glm::vec3 new_direction = _look_at - new_position;
	float new_distance = glm::length(new_direction);
	if(new_distance >= move_look_at_threshold && glm::dot(direction, new_direction) > 0.) {
		_position += delta_pos;
	}
